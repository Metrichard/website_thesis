...
import { WAITING_LIST } from 'app/app.constants';
...
export class WaitingListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterDataService.getFilterDataForPage(WAITING_LIST).subscribe(
      data => {
    ...
    this.pageDataService.getPageData(WAITING_LIST).subscribe(
      data => {
  ...
  savePageData() {
    if(this.pageData.id === '') {
      this.pageData.pageName = WAITING_LIST;
  ...
  saveFilter() {
    let newFilter = new FilterData('', WAITING_LIST, this.selectedTag)
    if(this.filter.id === '') {
