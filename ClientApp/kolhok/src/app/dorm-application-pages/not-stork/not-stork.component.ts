...
import { NOT_STORK } from 'app/app.constants';
...
@Component({
  selector: 'app-not-stork',
  templateUrl: './not-stork.component.html',
  styleUrls: ['./not-stork.component.css']
})
export class NotStorkComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filterDataService.getFilterDataForPage(NOT_STORK).subscribe(
      data => {
    ...
    this.pageDataService.getPageData(NOT_STORK).subscribe(
      data => {
  ...
  savePageData() {
    if(this.pageData.id === '') {
      this.pageData.pageName = NOT_STORK;
  ...
  saveFilter() {
    let newFilter = new FilterData('', NOT_STORK, this.selectedTag)
    if(this.filter.id === '') {
